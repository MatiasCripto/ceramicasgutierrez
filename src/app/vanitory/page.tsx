...
const SIZES = ['Todas', '60cm', '80cm', '100cm', '120cm']
const COLORS = ['Todos', 'Blanco', 'Negro', 'Gris', 'Madera', 'Beige']
const FINISHES = ['Todos', 'Mate', 'Brillante', 'Laqueado']

export default function VanitoryPage() {
  ...
  useEffect(() => {
    supabase
      .from('products')
      .select('id, name, description, category, size, color, finish, price_per_m2, price_per_unit, m2_per_box, images, attributes')
      .eq('category', 'vanitory')
      .eq('active', true)
      .order('name')
      .then(({ data }) => {
